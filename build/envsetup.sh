...
CHROMIUM_CUSTOM_BUILDFILES_REMOTE="${CHROMIUM_CUSTOM_BUILDFILES_REMOTE:-origin}"
# USE_WIGGLE defaults to n (off) except when sync_cromite is used.
USE_WIGGLE="${USE_WIGGLE:-}"
EXPORT_APPLIED_PATCHES="${EXPORT_APPLIED_PATCHES:-n}"
...
find_chromium_out_path() {
  ...
  if [ -n "${OUT:-}" ] && \
        [ "${OUT_AUTO_GENERATED:-}" = "${OUT:-}" ] && \
        [ "${OUT_CHROMIUM_TARGET_CPU:-}" != "${CHROMIUM_TARGET_CPU:-}" ]; then
    echo "Selecting a new OUT path due to CHROMIUM_TARGET_CPU change" >&2
    unset OUT
  fi
  if [ -z "${OUT:-}" ]; then
...
build() {
  ...
  (cd "$CHROMIUM_SRC_PATH" && time autoninja -k 0 -C "$OUT" trichrome_chrome_64_32_bundle trichrome_library_64_32_apk trichrome_webview_64_32_apk system_webview_shell_apk > >(tee "$OUT/build.log") && \
    ...
    echo "or type 'showlog' to see the full build log." >&2
    return $err
  }
}
  ...
  for possible_path in "${possible_paths[@]}"; do
    ...
    case "$(git -C "$resolved_path" remote get-url "$CHROMIUM_SRC_REMOTE")" in
      "$CHROMIUM_SRC_REMOTE_URI"*)
        ...
        echo "INFO: Auto-detected CHROMIUM_SRC_PATH as $CHROMIUM_SRC_PATH" >&2
        echo "If this is not correct, please set it manually." >&2
        return 0
        ;;
    esac
  done
...
find_brave_path() {
  ...
  if find_chromium_src_path; then
...
query_cromite_wip_branches() {
  CROMITE_WIP_BRANCHES="$(git_cromite ls-remote "$CROMITE_REMOTE" --refs --tags 'refs/heads/*next*')"
}
...
show_cromite_wip_branches() {
  [ -n "${CROMITE_WIP_BRANCHES:-}" ] || query_cromite_wip_branches || return $?
  printf "%s\n" "$CROMITE_WIP_BRANCHES" \
    | grep -v $'\t''refs/heads/next-v115$'   # an old next- branch still hanging around
  # We actually want to show the commit hash, too, not strip it out.
  # | sed -n -e 's!.*\trefs/heads/\(.*\)$!\1!p'
}
...
update_chromium_sources() {
  if ! which gclient >/dev/null; then
    echo "Could not find gclient. Is your environment prepared to build Chromium?" >&2
    echo >&2
    echo "See: https://www.chromium.org/developers/how-tos/get-the-code/" >&2
  fi
  local do_force=
  for arg in "$@"; do
    case "$arg" in
  ...
  _maybe_error_about_unset_chromium_version || return $?
  local err=0
  _maybe_warn_about_branch_already_exists || err=$?
  ...
  local err=0
  (cd "$CHROMIUM_SRC_PATH" && gclient validate) || err=$?
  if [ $err -ne 0 ]; then
    echo "----" >&2
    echo "Failed to validate gclient config." >&2
    echo "Typically, CHROMIUM_SRC_PATH should be the chromium/src directory. Currently: $CHROMIUM_SRC_PATH" >&2
    echo "Commands like 'gclient sync' and 'gclient runhooks' won't work in alternate worktrees." >&2
    echo "If this problem continues, and you are not using an alternative worktree like src2," >&2
    echo "you may need to start over using the upstream Chromium build guide." >&2
    echo >&2
    echo "See: https://www.chromium.org/developers/how-tos/get-the-code/" >&2
    return $err
  fi
  ...
  if git_chromium rev-parse "refs/heads/$calyx_branch" >/dev/null 2>&1; then
    git_chromium checkout --detach HEAD >/dev/null || return $?
    git_chromium branch -D "$calyx_branch" || return $?
  fi

  git_chromium checkout -b "$calyx_branch" "$V" || return $?
...
update_calyx_chromium_sources() {
  ...
  git_custom_buildfiles reset --hard HEAD >/dev/null || return $?
  git_custom_buildfiles clean -f >/dev/null || return $?
...
_should_use_wiggle() {
  if [ "${USE_WIGGLE,,:-}" = "auto" ]; then
    # Use wiggle if found.
    # NOTE: 'auto' is not the default.
    which wiggle >/dev/null 2>&1 || return 1 # false
    return 0 # true
  fi
  if is_var_true USE_WIGGLE n; then
    if ! which wiggle >/dev/null 2>&1; then
      echo "USE_WIGGLE=$USE_WIGGLE but could not find wiggle; skipping it." >&2
      echo "wiggle may greatly aid in patch application. You can find it at: https://github.com/neilbrown/wiggle" >&2
      echo "To prevent this message in the future, set USE_WIGGLE=n" >&2
      return 1 # false
    fi
    return 0 # true
  fi
...
output_patches() {
  ...
  upstream_branch="$(git -C "$CHROMIUM_SRC_PATH" rev-parse --abbrev-ref --symbolic-full-name @{u})" || return $?
  [ -n "$upstream_branch" ] || upstream_branch="${V:-}"
  [ -n "$upstream_branch" ] || { echo "Upstream branch expected. Set one or something!" >&2; return 2; }
  local left_right
  left_right="$(git -C "$CHROMIUM_SRC_PATH" rev-list --left-right --count "$upstream_branch"...HEAD)" || return $?
  ...
  git -C "$CHROMIUM_SRC_PATH" format-patch --full-index -N -k -P --zero-commit -"$ahead" || return $?
...
update_patches_lists() {
  find_chromium_custom_buildfiles_path || return $?
  (cd "$CHROMIUM_SRC_PATH" && ls -1 0*.patch | cut -d - -f 2- | sed -n "$FIRST_VENDOR_PATCH_REGEX"'q;p' > "$CHROMIUM_CUSTOM_BUILDFILES_PATH/01-cromite_patches_list.txt") || return $?
  (cd "$CHROMIUM_SRC_PATH" && ls -1 0*.patch | cut -d - -f 2- | sed -n "$FIRST_VENDOR_PATCH_REGEX"',$p' > "$CHROMIUM_CUSTOM_BUILDFILES_PATH/02-calyx_patches_list.txt")
}
...
move_all_patches() {
  find_chromium_src_path || return $?
  find_chromium_custom_buildfiles_path || return $?
  (cd "$CHROMIUM_SRC_PATH" && mv *.patch "$CHROMIUM_CUSTOM_BUILDFILES_PATH/patches/") || return $?
}
...
sync_cromite() {
  ...
  local err=0
  _maybe_warn_about_dirty_cromite || err=$?
  _maybe_warn_about_dirty_chromium_src || err=$?
  if [ $err -ne 0 ]; then
    if [ -z "$do_force" ]; then
      echo "ERROR: Not proceeding without --force" >&2
  ...
  _maybe_error_about_unset_chromium_version || return $?
  local calyx_branch="calyxos-$V"
  if ! git_chromium rev-parse "refs/heads/$calyx_branch" >/dev/null; then
    echo "Could not find branch $calyx_branch." >&2
    echo "Please run update_chromium_sources before trying to use version $V with sync_cromite." >&2
    return 1
  fi
  update_cromite_sources $do_force || return $?
  local err=0
  local our_patches
  ...
  # Remove all patches that we share with Cromite.
  (cd "$our_patches" && cat ../01-cromite_patches_list.txt | xargs -d'\n' rm) || return $?

  # Copy over all patches that we share with Cromite.
  cp "$their_patches/"*.patch "$our_patches/" || return $?

  # BEGIN SPECIAL CASES AND HANDLING OF DELETIONS/RENAMES.
  # Some of our Cromite patches have different names than upstream. Rename these.
  (
    cd "$our_patches"
  ...
  drop_to_shell "sync_cromite[pre]" "$our_patches"
  # END SPECIAL CASES AND HANDLING OF DELETIONS/RENAMES.

  git_custom_buildfiles clean -f || return $?

  # We have copied all our patches from Cromite verbatim.
  # Commit this for now.
  git_custom_buildfiles commit -a -m "WIP Sync with Cromite $CROMITE_VERSION" --allow-empty || err=$?
  if [ $err -ne 0 ]; then
    echo >&2
  ...
  # Switch to a new tmp branch, detaching ourselves in case we are currently on it.
  git_chromium checkout --detach HEAD >/dev/null 2>&1 || true
  ...
  # Output our current patches to CHROMIUM_SRC_PATH and copy their original Change-Id's from before
  # into the patch files that were exported.
  output_patches || return $?
  copy_existing_change_ids || return $?

  # Undo our revert of the sync, so the buildfiles dir has the latest Cromite patches again.
  git_custom_buildfiles reset --hard HEAD~1 || return $?

  # Update and output all our applied patches.
  update_patches_lists || return $?
  rename_all_patches || return $?
  move_all_patches || return $?

  # Buildfiles dir now has our updated patches. Commit it.
  git_custom_buildfiles commit -a --amend --no-edit --allow-empty

  # Apart from still being on the tmp branch, we could be done here, but...

  # Reapply patches we just exported onto clean calyxos-$V branch, and export them one last time.
  # This ensures that they *can* be applied and also may fix spacing related to added Change-Id's.
  git_chromium checkout "calyxos-$V" || return $?
  git_chromium reset --hard "$V" || return $?
  # Should not fail or require wiggle.
  apply_patches || return $?
  update_patches || return $?

  # Finally, write sync commit one last time, leaving an opportunity to edit.
  git_custom_buildfiles commit -a --amend --allow-empty
}
...
update_cromite_sources() {
  ...
  case "$CROMITE_VERSION" in
    ...
    v*)
      ...
      ;;
    *)
      git_cromite fetch "$CROMITE_REMOTE" "$CROMITE_VERSION" || return $?
      git_cromite checkout FETCH_HEAD || return $?
      ;;
  esac
}
...
drop_to_shell() {
  ...
  # Supply relevant environment variables to our new shell.
  USE_WIGGLE="${USE_WIGGLE:-}" \
  CHROMIUM_SRC_PATH="${CHROMIUM_SRC_PATH:-}" \
  CHROMIUM_CUSTOM_BUILDFILES_PATH="${CHROMIUM_CUSTOM_BUILDFILES_PATH:-}" \
  CROMITE_PATH="${CROMITE_PATH:-}" \
  BRAVE_PATH="${BRAVE_PATH:-}" \
  V="${V:-}" \
  CROMITE_VERSION="${CROMITE_VERSION:-}" \
  BV="${BV:-}" \
  BOV="${BOV:-}" \
...
is_var_true() {
  ...
  local var_name="$1"
  shift 1
  is_true "$var_value" "$var_name" "$@" || return $?
}
  ...
  case "${value,,}" in
    ...
    *)
      if ! is_false "$value"; then
        echo "warning: variable $name with value '$value' (orig '$orig_value') is not false or true; treating as false" >&2
      fi
      return 1
      ;;
  esac
