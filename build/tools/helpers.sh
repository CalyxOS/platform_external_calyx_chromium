...
build() {
  ...
  find_chromium_src_path || return $?
  if [ -z "${V:-}" ]; then
    echo "Please set V to the version"
    return 2
  fi
  ...
  if [ -z "$OUT" ]; then
    ...
    if [ -e "$OUT/args.gn" ]; then
      ...
      cp "${CHROMIUM_SRC_PATH}/args.gn" "$OUT/args.gn"
    fi
  fi
  (cd "$CHROMIUM_SRC_PATH" && gn gen "$OUT") || return $?
  (cd "$CHROMIUM_SRC_PATH" && time autoninja -k 0 -C "$OUT" trichrome_chrome_64_32_bundle trichrome_library_64_32_apk trichrome_webview_64_32_apk system_webview_shell_apk && \
    pushd "$OUT/apks"     && \
    rm -f universal.apk     && \
  ...
  if [ $err -ne 0 ]; then
    ...
    if [ $err -ne 0 ]; then
      if [ -n "$reject_arg" ]; then
        ...
        echo "You can use the 'e' command to edit unmerged/modified files." >&2
        echo "You can then use 'a' to check in your changes, and 'c' to continue." >&2
      else
        echo "Please review and apply the patch manually." >&2
        echo "Use the 's' command as a shortcut to view the patch." >&2
