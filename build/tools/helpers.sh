...
sync_cromite() {
  ...
  pushd "$our_patches" >/dev/null
  drop_to_shell "sync_cromite"
  popd >/dev/null

  #git_custom_buildfiles checkout -- PARTIAL-Multiple-fingerprinting-mitigations.patch

  git_custom_buildfiles clean -f || return $?
  git_custom_buildfiles commit -a -m "WIP Sync with Cromite $CROMITE_VERSION" || return $?
  ...
  EXPORT_APPLIED_PATCHES=n apply_patches || \
  {
    err=$?
    while true; do
      sleep 1 || return $?
      echo
      echo "Error $err, something went wrong." >&2
      echo "Please rectify the situation and then type 'exit' to continue." >&2
  ...
  # fix anything wrong if needed, and apply_patches_resume
...
### UTILITIES ###
drop_to_shell() {
  bash --init-file <(printf "PS1=%q; source %q\n" "${1:-fixme}"'$'" " "${BASH_SOURCE[0]}")
}
